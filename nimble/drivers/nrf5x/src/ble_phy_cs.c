...
extern uint8_t g_ble_phy_cs_lock;
...
static inline void
ble_phy_cs_rtt_set(uint8_t *rtt_segments)
{
    if (rtt_segments == NULL) {
        NRF_RADIO->RTT.SEGMENT01 = 0;
        NRF_RADIO->RTT.SEGMENT23 = 0;
        NRF_RADIO->RTT.SEGMENT45 = 0;
        NRF_RADIO->RTT.SEGMENT67 = 0;
    } else {
        NRF_RADIO->RTT.SEGMENT01 = rtt_segments[0];
        NRF_RADIO->RTT.SEGMENT23 = rtt_segments[1];
        NRF_RADIO->RTT.SEGMENT45 = rtt_segments[2];
        NRF_RADIO->RTT.SEGMENT67 = rtt_segments[3];
    }
}
        ...
        if (t->mode == BLE_PHY_CS_TRANSM_MODE_SYNC) {
            ...
            if (t->is_tx) {
                ...
                if (NRF_RADIO->PREFIX0 & 0x80) {
                    g_ble_phy_tx_buf[0] = 0b10101010;
                } else {
                    g_ble_phy_tx_buf[0] = 0b01010101;
                }
