...
phy_ppi_init(void)
{
    ...
    NRF_TIMER00->SUBSCRIBE_CAPTURE[2] = DPPI_CH_SUB(DPPIC00_RADIO_EVENTS_END);

    /* Link events between RADIO and MCU power domains */
    PPIB_MCU_RADIO_0(DPPIC00_TIMER00_EVENTS_COMPARE_0, TIMER0_EVENTS_COMPARE_0);
    PPIB_MCU_RADIO_1(DPPIC00_TIMER00_EVENTS_COMPARE_3, TIMER0_EVENTS_COMPARE_3);
    PPIB_RADIO_MCU_0(RADIO_EVENTS_ADDRESS, DPPIC00_RADIO_EVENTS_ADDRESS);
    PPIB_RADIO_MCU_1(RADIO_EVENTS_END, DPPIC00_RADIO_EVENTS_END);
#else /* TIMER00 only for ToF measurement */
