...
static struct ble_ll_cs_step_transmission transmission_pattern[300];
static struct ble_ll_cs_step subevent_steps[76];
...
#if MYNEWT_VAL(BLE_LL_CHANNEL_SOUNDING_USE_CUSTOM_CHANNELS)
static uint8_t custom_channel_ids[] = MYNEWT_VAL(BLE_LL_CHANNEL_SOUNDING_CUSTOM_CHANNELS);
#endif
...
{
#if MYNEWT_VAL(BLE_LL_CHANNEL_SOUNDING_USE_CUSTOM_CHANNELS)
    if (cssm->mode0_next_chan_id > ARRAY_SIZE(custom_channel_ids)) {
        cssm->mode0_next_chan_id = 0;
    }
    step->channel = custom_channel_ids[cssm->mode0_next_chan_id++];
#else
    int rc;
                        ...
                        uint8_t subevent_state, uint16_t steps_in_procedure_count)
{
    ...
    last_transm_slot = transmission_pattern + ARRAY_SIZE(transmission_pattern) - 1;
...
ble_ll_cs_proc_subevent_generate(struct ble_ll_cs_sm *cssm)
{
    ...
    while (true) {
        ...
        if (cssm->steps_in_procedure_count + steps_in_subevent > BLE_LL_CS_STEPS_PER_PROCEDURE_MAX ||
            ...
            rc = ble_ll_cs_setup_next_procedure(cssm);
            if (rc) {
                cssm->number_of_generated_steps = 0;
    ...
    cssm->number_of_generated_steps = steps_in_subevent;

    return 0;
}

int
ble_ll_cs_step_end(struct ble_phy_cs_transmission *prev_transm,
                   struct ble_phy_cs_tone_results *tone_results,
                   struct ble_phy_cs_sync_results *sync_results)
{
    ...
    BLE_LL_ASSERT(&step->last_transm->phy_transm == prev_transm);

    if (tone_results) {
        ble_ll_cs_tone_results_set(step, tone_results);
    }

    if (sync_results) {
        ble_ll_cs_sync_results_set(step, sync_results);
    }
...
ble_ll_cs_proc_schedule_next_subevent(struct ble_ll_cs_sm *cssm)
{
    ...
    cssm->sch.end_time = anchor_cputime + ble_ll_tmr_u2t_up(params->subevent_len);
...
ble_ll_cs_proc_scheduling_start(struct ble_ll_conn_sm *connsm, uint8_t config_id)
{
    ...
    rc = ble_ll_cs_proc_schedule_first_subevent(cssm);
    if (rc) {
        return BLE_ERR_UNSPECIFIED;
    }
